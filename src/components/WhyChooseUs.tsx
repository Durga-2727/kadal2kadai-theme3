...
export default function WhyChooseUs() {
  const icons = [
    <Ship className="h-6 w-6 text-[#64748B]" />,
    <Sun className="h-6 w-6 text-[#64748B]" />,
    <Award className="h-6 w-6 text-[#64748B]" />
  ];
  ...
  return (
    <section id="why-choose-us" className="py-10 sm:py-12 bg-slate-50 relative overflow-hidden select-none">
      ...
      <div className="absolute top-0 left-0 w-full overflow-hidden leading-none rotate-180 opacity-5">
        <svg viewBox="0 0 1200 120" preserveAspectRatio="none" className="relative block w-full h-[60px]" style={{ fill: '#64748B' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-2xl mx-auto mb-6 sm:mb-8">
          <span className="text-[#64748B] text-xs font-bold font-mono tracking-widest uppercase block mb-2">
            The Golden Standard
          </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {WHY_CHOOSE_US.map((block, idx) => (
            <motion.div
              ...
              className="bg-white p-8 rounded-2xl border border-slate-100 hover:border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 flex flex-col items-start gap-4 relative overflow-hidden group"
            >
              <div className="absolute top-0 right-0 w-24 h-24 bg-[#64748B]/5 rounded-bl-[100px] transform translate-x-4 -translate-y-4 group-hover:scale-110 transition-transform" />
              
              <div className="p-3.5 bg-slate-50 text-slate-800 rounded-xl group-hover:bg-[#64748B] group-hover:text-white transition-all duration-300 shadow-sm">
                {icons[idx] || <Ship className="h-6 w-6" />}
              </div>

              <div>
                <h3 className="font-serif text-lg font-bold text-slate-800 tracking-tight group-hover:text-[#64748B] transition-colors mb-2">
                  {block.title}
                </h3>
